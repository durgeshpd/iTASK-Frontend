...
import { toggleMenu } from "../redux/appSlice"; // Import toggleMenu
...
const Navbar = () => {
  ...
  const handleToggleMenu = () => {
    console.log("Hamburger menu clicked");  // Debugging the toggle action
    dispatch(toggleMenu()); // Dispatch action to toggle the sidebar
  };

  return (
    <div className="navbar bg-base-300 shadow-sm px-4">
      {/* Hamburger Menu (visible on all screen sizes for testing) */}
      <div className="block">
        <button onClick={handleToggleMenu} className="btn btn-ghost text-xl text-base-content">
          <span className="material-icons">menu</span> {/* Ensure Material Icons are loaded */}
        </button>
      </div>
